...
- A REPOT `care.done` (and `substrate.refresh`) is DESTRUCTIVE, not additive: it clears the named plant's stored calibration anchors and retracts every derived soil-wetness reading from the refreshed date forward, because a reading computed against the old fill describes a pot that no longer exists. Propose either only when the medium genuinely changed.
- Omitting `charged` on `substrate.refresh` does NOT preserve whatever charge state was recorded earlier — it clears it to NULL ("derive from the mix"). Set it explicitly only when you actually know the medium's nutrient state.
- A `care.done` for a one-per-day task (`WATER`, `FERTILIZE`, `REPOT`, `ROTATE`, `CLEAN_LEAVES` — never `MIST`) still succeeds on a repeat, but its result is `already-recorded-on-day` naming the day the submission itself named (day-scoped, never "today" specifically) instead of `applied` — read it so you do not re-propose a no-op. On REPOT, that result's `otherEffectsApplied: true` means the profile, substrate and schedule still updated even though no new care event was written.
